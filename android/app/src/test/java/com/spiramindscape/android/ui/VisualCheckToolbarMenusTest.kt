...
import com.spiramindscape.android.ui.components.MenuChoiceVariant
 ...
 *
 * This is the picture that matters for the 2026-08-15 redesign: the menus are **stacked groups,
 * each under its heading with a hairline beneath it** (no vertical divider), the Direction rows are
 * the `Aux` pale-grey look with the ascending/descending glyphs, and the whole card still fits the
 * phone. It also shows the drop shadow, which used to be `shadowElevation = 12.dp` — a hard grey
 * band round the card rather than depth.
 *
                        ...
                        SpiraMenuColumns {
                            ...
                            SpiraMenuGroup("Direction") {
                                SpiraMenuChoice(
                                    ...
                                    {},
                                    selected = true,
                                    variant = MenuChoiceVariant.Aux,
                                    icon = SpiraIcons.SortAscending,
                                )
                                SpiraMenuChoice(
                                    "Descending",
                                    {},
                                    variant = MenuChoiceVariant.Aux,
                                    icon = SpiraIcons.SortDescending,
                                )
                            }
                        }
                    }
                    SpiraMenuSurface {
                        SpiraMenuColumns {
                            SpiraMenuGroup("Status") {
                                listOf(TargetFilter.All, TargetFilter.Done, TargetFilter.NotDone).forEach {
                                    SpiraMenuChoice(it.label, {}, selected = it == TargetFilter.All)
                                }
                            }
                            SpiraMenuGroup("Progress") {
                                listOf(TargetFilter.Started, TargetFilter.NotStarted).forEach {
                                    SpiraMenuChoice(it.label, {}, selected = false)
                                }
                            }
