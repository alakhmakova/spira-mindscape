...
function GoalsOverview() {
  ...
  // IDs of goals that passed current filters — GoalsTable uses this to hide orphaned targets/tasks
  const filteredGoalIds = useMemo(() => new Set(filtered.map((g) => g.id)), [filtered]);
      ...
      <div className="spira-overview-inner mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-12 space-y-8">
        <header className="spira-overview-header flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
            <div className="inline-flex w-full sm:w-auto p-0.5 bg-secondary rounded-md border hairline">
              ...
              <Link
                to="/calendar"
                className="flex-1 sm:flex-none px-3 h-9 rounded-md text-xs flex items-center justify-center gap-1.5 transition-colors font-medium text-muted-foreground hover:text-foreground"
              >
...
function ViewBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex-1 sm:flex-none px-3 h-9 rounded-md text-xs flex items-center justify-center gap-1.5 transition-colors font-medium",
        active
